...
export async function GET(req: NextRequest) {
  ...
  const token = process.env.BIO_UPLOAD_TOKEN;
  const token2FA = process.env.BIO_UPLOAD_2FA_TOKEN;
  ...
  const requiredHeader = req.headers.get("X-Auth-Upload-Pics");

  if (!requiredHeader || requiredHeader !== token2FA) {
    return NextResponse.json(
      {
